...
NO markdown fencing around the JSON. Return raw JSON only.`,
    ...
    userTemplate: (goal: string, context: string) => 
...
Return ONLY the raw code. No markdown fencing (\`\`\`). No explanations. Just the code.`,
    ...
    userTemplate: (filePath: string, description: string, interfaceContext: string) =>
...
Return ONLY the raw test code. No markdown fencing. No explanations.`,
    ...
    userTemplate: (sourceCode: string, importPath: string) =>
      `## Source Code to Test
\`\`\`typescript
${sourceCode}
\`\`\`
...
4. Apply appropriate fix`,
    ...
    userTemplate: (
      ...
      const fixerPrompts: Record<string, string> = {
...
Return properly formatted Markdown.`,
    ...
    readmeTemplate: (reasoning: string, tasks: Array<{ file_path: string; description: string }>) =>
...
\`\`\``,
    ...
    userTemplate: (specification: string) =>
      `## UI Specification
${specification}
 ...
 */
export function getFixerPrompt(errorCategory: string): string {
