...
import { IFileSystem, IStateManager, ExecutionState, TaskState } from '../interfaces/index.js';
...
export class StateManager implements IStateManager {
   ...
   */
  static generatePlanId(goal: string): string {
   ...
   */
  async findLatestForGoal(goal: string): Promise<ExecutionState | null> {
    const planIds = await this.list();
    let latestState: ExecutionState | null = null;
    let latestTime = 0;
  ...
  async cleanup(keepCount: number = 10): Promise<number> {
    ...
    const states: { planId: string; timestamp: number }[] = [];
    for (const planId of planIds) {
      const state = await this.load(planId);
      if (state) {
   ...
   */
  static createInitialState(goal: string): ExecutionState {
    return {
   ...
   */
  static getResumableTasks(state: ExecutionState): TaskState[] {
   ...
   */
  static isComplete(state: ExecutionState): boolean {
   ...
   */
  static isFatallyFailed(state: ExecutionState): boolean {
