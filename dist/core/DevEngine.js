...
export class DevEngine {
    ...
    constructor(adapter, options = {}) {
        ...
        this.scheduler = new TaskScheduler(adapter, this.executeTaskLogic.bind(this), schedulerOpts);
    ...
    async execute(context) {
        const goal = context.parameters.goal;
        const repoPath = context.parameters.repoPath;
        const resume = context.parameters.resume;
        try {
     ...
     */
    async run(goal, repoPath, resume) {
    ...
    async resumeExecution(state) {
        ...
        const schedulerTasks = state.tasks.map(t => ({
            id: t.id,
            filePath: t.filePath,
            description: t.description,
            dependencies: t.dependencies,
        ...
        // Resume scheduler
        this.scheduler.loadPlan(schedulerTasks);
        this.scheduler.resumeFrom(Array.from(completedResults.keys()), completedResults);
        await this.events.emit('phase:start', { phase: 'execution' });
        this.currentState.phase = 'executing';
        await this.scheduler.run();
     ...
     */
    async phaseArchitect(goal, context) {
     ...
     */
    async phaseDocument(plan) {
        this.adapter.log('Phase: Documentation');
        const content = await this.adapter.llm.generate({
            systemPrompt: PROMPTS.SCRIBE.system,
            userPrompt: PROMPTS.SCRIBE.readmeTemplate(plan.architecture_reasoning, plan.tasks.map(t => ({ file_path: t.file_path, description: t.description })))
        });
     ...
     */
    async runVerificationLoop(task, initialCode) {
     ...
     */
    async attemptFix(task, code, testCode, errorOutput, category) {
        this.adapter.log(`Fixer: Attempting to fix ${category} error`);
        const fixerPrompt = getFixerPrompt(category);
        const response = await this.adapter.llm.generate({
            systemPrompt: fixerPrompt,
            userPrompt: PROMPTS.FIXER.userTemplate(task.filePath, errorOutput, code, testCode, category)
        });
        return this.cleanCodeResponse(response);
    }
    // ===========================================================================
