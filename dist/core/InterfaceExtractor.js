...
export class InterfaceExtractor {
     ...
     */
    extractSymbols(sourceFile, options = {}) {
        const symbols = [];
        const visit = (node) => {
     ...
     */
    extractNode(node, sourceFile, options) {
     ...
     */
    printInterface(node, sourceFile) {
     ...
     */
    printClassSignature(node, sourceFile, options) {
        const parts = [];
     ...
     */
    extractMemberSignature(member, sourceFile, options) {
     ...
     */
    printConstructorSignature(node, sourceFile) {
     ...
     */
    printMethodSignature(node, sourceFile) {
        const modifiers = this.getModifiersText(node);
     ...
     */
    printPropertySignature(node, sourceFile) {
        const modifiers = this.getModifiersText(node);
        const name = node.name.getText(sourceFile);
     ...
     */
    printAccessorSignature(node, sourceFile) {
     ...
     */
    printFunctionSignature(node, sourceFile) {
        const modifiers = this.getModifiersText(node);
     ...
     */
    printVariableSignature(node, sourceFile) {
        const modifiers = this.getModifiersText(node);
